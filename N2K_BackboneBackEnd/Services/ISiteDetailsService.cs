...
namespace N2K_BackboneBackEnd.Services
{
    public interface ISiteDetailsService
    {
        ...
        Task<List<StatusChanges>> AddCommentBulk(string sitecodes, string comment, IMemoryCache cache);
        Task<int> DeleteComment(long CommentId, IMemoryCache cache, bool temporal = false);
        Task<int> DeleteCommentBulk(string sitecodes, string comment, IMemoryCache cache);
        Task<List<StatusChanges>> UpdateComment(StatusChanges comment, IMemoryCache cache, bool temporal = false);
        Task<List<StatusChanges>> UpdateCommentBulk(string sitecodes, string oldComment, string newComment, IMemoryCache cache);
